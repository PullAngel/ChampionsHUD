...
- ~~**Duplicar un cálculo** para comparar variantes~~ — **HECHO, 2026-08-11.** Angel decidió dónde vive el historial: *"que se guarde temporalmente, no como los teams. Que no desaparezca al cambiar de la calculadora, pero que no se guarde por siempre"*. Botón **Duplicar** en Resultado congela el cálculo actual (atacante/defensor/movimiento/veredicto ya resuelto, autocompletado incluido) como una fila en una tarjeta nueva "Comparar", debajo del Resultado. `CALC_HISTORY` vive en memoria, fuera de `B`/`MY`, nunca pasa por `save()` — sobrevive cambiar de pestaña dentro de la app abierta, se pierde al recargarla (mismo patrón que `SPD_WHATIF`, decisión #25). Cada fila tiene su ✕ para borrarla sola. `calcResolve(c)` (nuevo, `hud.html`) factoriza la cadena de autocompletado que antes solo vivía adentro de `vCalc()`, para que "Duplicar" congele EXACTAMENTE el número en pantalla y no un recálculo aparte que se pueda desincronizar. Es una foto del resultado, no una configuración editable — no hay "cargar de nuevo para seguir editando" en esta primera versión. 3 tests nuevos.
