...
### 24. El HUD le habla al jugador, no al desarrollador — y la confianza se ve además de leerse
**Contexto:** con Fase 2 cerrada, Angel pidió revisar el tono y el lenguaje de todo lo que el HUD muestra. Se extrajeron los 40 títulos de tarjeta, 24 mensajes, 13 avisos y 11 subtítulos reales del proyecto y se leyeron desde afuera. El diagnóstico encontró cuatro problemas, todos con evidencia:
1. **La app le habla al desarrollador** en 8 lugares — pide correr `build_dex.py`, subir un `meta.json` a Drive, o "mandame esta pantalla".
2. **La app explica su propia filosofía de diseño**: *"por evidencia, no por puntaje"* contrasta contra una versión anterior que el jugador nunca vio; *"Sin orden ni elección: los datos son tuyos para decidir"* es literalmente la decisión #21 escrita en pantalla; *"Hechos observados, sin interpretar"* describe la arquitectura del event log, no lo que el jugador gana con ella.
3. **Los íconos del riel no significan nada** (`◈ ▦ ◆ ● ÷`), y los dos más usados — Rival y Mío — eran casi la misma forma.
4. **No existe "la primera vez"**: la pantalla de inicio pide tres permisos y desaparece; el HUD abre con doce tarjetas simultáneas.

Se le presentaron a Angel tres rutas (voz / confianza visual / onboarding) y eligió **A+B juntas, en una sola pasada, con criterio**, con dos correcciones explícitas sobre lo propuesto.

**Decisión:**
1. **Todo texto visible se escribe desde el lado del jugador.** Se nombra lo que la persona reconoce, no cómo está construido el sistema. Nada de nombres de archivo, scripts ni rutas en la UI normal. El HUD no explica sus propias decisiones de diseño: si una regla del proyecto (ej. #21) está bien aplicada, se nota en que la interfaz no manda — no hace falta anunciarlo.
2. **La confianza se muestra con marca visual Y texto corto, no con uno solo.** Corrección explícita de Angel sobre la propuesta original, que eliminaba el texto: *"que sean las 2 cosas, manteniendo textos cortos y entendibles"*. El razonamiento es de curva de aprendizaje — la marca sola es más rápida de leer pero hay que aprenderla primero, y estamos optimizando justamente para quien todavía no la aprendió. Tres niveles, siempre los mismos en todo el HUD: **visto** (hecho observado), **deducido** (se descartó el resto), **probable** (prior de meta). Mapea 1:1 con los tres niveles de conocimiento de `inference.md` §1, que ya existían en el motor desde el sprint 2.2.
3. **Los errores van al revés que el resto.** Todo el texto normal se acorta y se humaniza; los errores **se vuelven más explícitos**: mensaje humano arriba, y debajo el detalle técnico con un código estable (`OCR-02`, `DATA-01`) que se puede buscar en el código. **Esto es deliberado y temporal** — ver `future.md`, "Limpieza de errores técnicos para la versión final".

**Consecuencias:** `vision.md`/`product.md` ganan un criterio de redacción explícito. El test de imperativos que ya protege #21 se amplía para cubrir los strings reescritos — es la red que evita que "más humano" se deslice a "te digo qué hacer". Las decisiones #1/#19/#21 no se tocan: esta trata de *cómo se dice*, no de *qué se puede decir*.

---

### 25. "Qué pasaría si": una capa de hipótesis explícitamente separada del event log
**Contexto:** Angel pidió poder explorar el orden de velocidad bajo Pañuelo Elección, Viento Afín y fases de estadística (−6 a +6) sin haber confirmado ninguna de esas cosas todavía — "para poder pensar en diferentes tipos de estrategias". Esto es distinto de todo lo que el HUD hacía hasta ahora: cada dato que entra al sistema (ítem confirmado, habilidad vista, orden observado) se registra en el event log porque *pasó de verdad* (`architecture.md` §0, "event log inmutable como fuente única de verdad"). Un "¿y si tuviera Pañuelo?" no pasó — es una pregunta, no un hecho.
**Decisión:** los interruptores de "qué pasaría si" viven en `SPD_WHATIF`, una estructura en memoria completamente separada de `B` (estado de la partida) y `MY` (equipo propio). No pasa por `logEvent()`, no se serializa en `save()`/`saveMy()`, y se vacía sola (`resetSpdWhatIf()`) apenas entra un equipo rival nuevo — nunca sobrevive a la partida para la que se armó. Alcance: del lado rival se puede probar Pañuelo Elección, Viento Afín y fases, porque las tres son hipótesis genuinas sobre un Pokémon que todavía no se conoce del todo. Del lado propio solo Viento Afín y fases — el ítem real ya está reflejado en el número, agregar un "Pañuelo hipotético" ahí sería inventarle un segundo ítem a un Pokémon que ya tiene uno confirmado. La megaevolución, cuando existe, ignora el Pañuelo hipotético al calcularse: son mecánicamente incompatibles (la piedra ocupa el slot de ítem), y modelarlo distinto sería mostrar una combinación imposible en el juego real.
**Consecuencias:** cualquier feature futura de "qué pasaría si" (daño hipotético, HP hipotético, etc.) tiene un precedente claro de dónde vivir: nunca en `B`/`MY`, siempre en su propia estructura transitoria, documentada como tal en el código. Esto también fija el vocabulario: "confirmado" y "visto" siguen significando exactamente lo mismo que antes (decisión #24) — un interruptor de esta capa nunca se disfraza de evidencia real, ni en el dato ni en el texto que lo describe.
**Estado:** Aceptada, 2026-08-05. Implementado en "Quién pega primero" (Previa) — `stageMul()`, `applyWhatIf()`, `whatIfMine()`/`whatIfFoe()`, `resetSpdWhatIf()` en `hud.html`. La vista "Detallada" (rangos ponderados por spread/naturaleza real vía `speedWeights()`, eje de velocidad horizontal, mismos interruptores) también implementada, 2026-08-06 — ver `roadmap.md`, sprints 3.1/3.2.
**Estado:** Aceptada, 2026-08-05, con elección explícita de Angel entre tres rutas y dos correcciones suyas sobre la propuesta. Ver `future.md` para los cuatro trabajos que quedaron diferidos a propósito.

---

### 26. Fork de diseño futuro: mismo Motor, Presentación nueva desde cero
**Contexto:** Angel planea, más adelante, crear una versión distinta del producto que reuse el mismo Motor, algoritmos e ideas de Champions HUD (incluso mejorándolos si hace falta), pero con un **diseño de interfaz completamente nuevo**, hecho desde Claude Design y sin heredar ninguna decisión de Presentación ya tomada (Glance/Peek/Deep, el sistema de confianza visual de la decisión #24, la disposición de paneles, etc.) — para poder comparar las dos versiones y quedarse con la que resulte más simple de usar. Esto no es "otra plataforma" en el sentido de la decisión #10 (mismo Motor, mismo diseño, otro sistema operativo) — es **el mismo Motor con un diseño deliberadamente distinto**, en principio incluso en el mismo Android. La diferencia importa: la decisión #10 ya garantiza portabilidad de plataforma; esta decisión garantiza portabilidad de *diseño*.
**Decisión:** no se construye el fork ahora — sigue siendo una intención a futuro, sin fecha, y no condiciona el trabajo actual salvo en un punto: **la arquitectura tiene que mantenerse "amable" para ese fork**, es decir, que el día que se arranque, el trabajo sea diseñar una Presentación nueva sobre el Motor existente, no desenredar el Motor de la Presentación actual primero. Dos cosas ya cumplen este rol y se refuerzan en vez de relajarse:
- **El corte que usa `tests/run.js`** (todo lo que vive antes de `function vPre(){` en `hud.html`) ya es, en la práctica, la frontera Motor/Presentación validada por 223 tests que corren sin DOM ni Android — es la prueba de que ese corte es real, no aspiracional.
- **El adaptador `IO`** (`hud.html`, cerca de la definición de `has`, ver `architecture.md` §7.4): las 8 llamadas de entrada/salida que el Motor hace al puente Android (`loadDex`, `haptic`, `keepOpen`, `saveBattle`, `loadTeam`, `saveTeam`, `loadMeta`, `loadBattle`) estaban sueltas por el archivo, cada una detrás de su propio `try/catch`. Se agruparon en un único objeto con nombre — el mismo comportamiento de siempre, pero ahora es un solo lugar para reimplementar si un cliente nuevo necesita otro backend de persistencia (por ejemplo, un mock de Node para el propio `tests/run.js`, o `localStorage` si el fork corriera en un navegador en vez de dentro del WebView de Android). Un test permanente (`tests/run.js`, "las 8 llamadas... viven TODAS adentro de IO") falla si alguien agrega una llamada suelta por fuera de ese objeto.
**Consecuencias:** ningún trabajo nuevo se bloquea por esto — es una restricción de estilo sobre refactors que ya estaban planeados (Fase 3), no una feature que agregar. Cuando el fork arranque de verdad, es trabajo de Presentación pura: una carpeta/rama nueva que importe el mismo `hud.html` recortado hasta `vPre()` (o lo que en ese momento sea el módulo del Motor) y construya su propia capa de vistas desde cero, con su propio sistema de diseño. El Motor no se toca para que el fork exista — si en algún punto hace falta tocarlo (mejorar un algoritmo, agregar una capacidad), esa mejora beneficia a los dos clientes por igual, porque siguen siendo el mismo Motor.
**Estado:** Aceptada, 2026-08-11. Sin implementación de Presentación todavía — es una intención documentada. Primer paso de refuerzo de arquitectura hecho el mismo día: adaptador `IO`, ver `architecture.md` §7.4 y `roadmap.md` (Fase 3).
