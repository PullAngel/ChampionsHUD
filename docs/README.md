...
**Actualizado 2026-08-11.** Las tres primeras fases están cerradas: **Fase 0 — Estabilización** (2026-07-31), **Fase 1 — Fricción crítica de uso** (2026-08-03, confirmada por Angel en dispositivo real) y **Fase 2 — Datos de meta reales y motor de inferencia** (2026-08-04). La **Fase 3** (formalizar la separación Motor/Cliente) tiene su criterio de salida cumplido de hecho (medido 2026-08-06, ver `roadmap.md`), pero el refactor estructural queda pendiente a propósito hasta poder probarlo con Angel en dispositivo. De la **Fase 4** (memoria y análisis post-combate) se hizo la mitad que ya tenía arquitectura cerrada — arrastrar lo confirmado del rival entre juegos de un Bo3 (2026-08-11) —; el resumen post-partida sigue sin empezar, es una conversación de producto pendiente.
...
**Cómo se corren los tests: `node tests/run.js`, y nada más.** Esa única orden ejecuta los 223 casos de JS, `validate_data.py`, y los 36 casos de Python de los generadores (`tests/test_build_meta*.py`) — estos últimos existían desde la Fase 2 y **nadie los estaba corriendo** hasta el 2026-08-06. Si se agrega un archivo de tests nuevo, engancharlo ahí: una red de seguridad que hay que acordarse de tirar a mano no es una red.
