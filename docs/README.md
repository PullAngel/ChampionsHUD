...
**Cómo se corren los tests: `node tests/run.js`, y nada más.** Esa única orden ejecuta los 218 casos de JS, `validate_data.py`, y los 36 casos de Python de los generadores (`tests/test_build_meta*.py`) — estos últimos existían desde la Fase 2 y **nadie los estaba corriendo** hasta el 2026-08-06. Si se agrega un archivo de tests nuevo, engancharlo ahí: una red de seguridad que hay que acordarse de tirar a mano no es una red.
