...
4. **Partir `hud.html` en módulos** con contratos documentados (`@typedef` mínimo, `architecture.md` §8) entre capas — Percepción, Estado, Inferencia, Cálculo, Insights, Presentación. **Deliberadamente no abordado en esta sesión** — es el ítem más grande y riesgoso de la Fase 0 (reestructurar ~1500 líneas que hoy comparten un único scope global, y verificar que un WebView de Android cargando desde `file:///android_asset/` sirva módulos ES sin las restricciones de CORS que sí aplican en algunos navegadores) y se prefirió no apurarlo sin poder probarlo en un dispositivo real. Candidato para la próxima sesión, dedicada solo a esto.
...
**Criterio de salida:** 7 de 8 ítems resueltos. Queda pendiente únicamente el ítem 4 (modularización) — es una mejora estructural de mantenibilidad, no un bug activo: todos los hallazgos de `audit.md` §5 que describían comportamiento incorrecto ya están resueltos, y hay pruebas (`tests/run.js`, `validate_data.py`) que impedirían que se reintroduzcan en silencio.
