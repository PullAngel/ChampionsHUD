...
1. **Unificar habilidades a slugs canónicos en inglés** (`audit.md` §5.2, decisión #7). `calc()` compara contra 15+ literales en español; `build_dex.py` entrega inglés. Introducir la capa de slugs (`intimidate`, no `"Intimidación"`) en el motor de daño y mover el español a la capa de presentación vía `t()`. *Verificación:* generar `dex.json` con `build_dex.py` y correr un combate con una habilidad clave (p. ej. Intimidate) — el modificador debe aplicarse con datos de `dex.json`, no solo con las tablas embebidas.
2. **Regenerar `meta.json`** (`audit.md` §5.3) para que coincida con las tablas canónicas `IT`/`ABIL` ya corregidas (sin ítems ni habilidades inexistentes en Champions). La re-auditoría dejó la lista exacta de entradas huérfanas (15 ítems, 5 habilidades) como punto de partida. *Verificación:* cada entrada de `meta.json` resuelve contra un ítem/habilidad real de las tablas canónicas — este chequeo puede ser el primer caso de uso del validador del punto 5.
...
5. **Construir el sistema i18n desde cero, o descartar formalmente la idea por ahora** (`audit.md` §5.5, decisión #7). La re-auditoría encontró que no existe ninguna tabla `STRINGS` ni función `t()` en el código actual — no es una adopción parcial que completar, es una decisión a tomar: o se construye la capa de traducción completa antes de seguir agregando texto hardcodeado, o se documenta explícitamente en `decisions.md` que se pospone y se saca de la Fase 0.
6. **Escribir el script de validación de datos en build** (`architecture.md` §6): verifica que `meta.json`, `dex.json`, `sprite_index.json` y las tablas embebidas son mutuamente consistentes. Se apoya en el trabajo del punto 2 y sirve para detectar automáticamente una futura desincronización.
7. **Dejar una suite de pruebas mínima en el repo** (no scripts ad-hoc descartables, `audit.md` §5.7): como mínimo, un caso para el motor de daño con una habilidad clave (cubriría 5.2 si se reintrodujera), y el validador del punto 6 corriendo como test.
8. **Investigar qué reemplazó (si algo lo hizo) a la reconexión automática de permiso de captura** (`audit.md` §5.8, hallazgo nuevo). `ReconnectActivity` ya no existe en el repo; hace falta trazar `dead` en `ScreenCapture.kt`/`OverlayService.kt` para confirmar si la app se recupera sola de una revocación de permiso en Android 14+ o si quedó sin ese mecanismo. Decidir severidad recién después de esta investigación.
9. **Limpieza menor:** quitar la dependencia declarada sin uso `kotlinx-coroutines-android` del `build.gradle.kts` si no se va a usar, o empezar a usarla si simplifica el manejo de hilos existente con `HandlerThread`/`Handler` (`audit.md` §5.6) — es la más baja prioridad de la fase, hacerla solo si sobra tiempo.

**Criterio de salida:** los hallazgos de `audit.md` §5 están resueltos (o formalmente pospuestos con una decisión explícita, como podría pasar con el punto 5) y hay pruebas que impedirían que se reintroduzcan en silencio.
