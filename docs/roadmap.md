...
**Reformulada el 2026-07-31** tras revisar el primer intento con Angel: la importación de equipo por texto no era lo que esperaba — quiere captura automática (OCR) de la pantalla "View Details" del juego, como ya funciona para el rival en team preview. Se replanteó el alcance con él (ver plan de esa fecha) en tres partes, en el orden que eligió: **A. equipos guardados, B. Previa, C. Campo — todas hechas.** La captura OCR del equipo propio queda **deliberadamente afuera**, para una sesión dedicada aparte (ver más abajo).

- ~~**A. Equipos guardados**~~ — **Hecho, 2026-07-31.** Guardar, nombrar, elegir, duplicar y borrar equipos completos (antes solo existía un único equipo sin nombre). Formato de persistencia v1→v2 con migración automática, sin tocar Kotlin. El importador de texto (`parseTeamText()`) queda como estaba — no se descartó, sigue siendo la forma rápida de cargar/corregir un equipo sin usar los selectores uno por uno, y va a servir también para corregir lo que la OCR futura lea mal.
...
**Pendiente de verificar en dispositivo real, sin excepción** (ninguno de estos cambios se pudo compilar ni probar acá — sin `gradlew` ni Android SDK accesible desde el entorno): el fix de teclado (`Bridge.needsKeyboard()` en `OverlayService.kt`, necesario para el importador de texto Y para el campo de URL de meta.json que ya existía), y toda la UX nueva de Campo/Previa/equipos guardados — la lógica está testeada (`tests/run.js`), pero el layout, el flujo táctil y que todo se sienta tan simple como se pidió necesitan una vuelta de ajuste real.

**Fuera de esta fase, para una sesión dedicada aparte:** captura OCR del equipo propio desde "View Details" (dos pantallas — Moves & More y Stats). No existe ningún OCR en el proyecto hoy; requeriría una dependencia nueva (Google ML Kit Text Recognition, on-device, ~20MB más de APK) y trabajo iterativo que solo se puede ajustar probando contra la pantalla real del juego.
