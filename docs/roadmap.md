...
3. Decidir el modelo de datos exacto de `MetaSnapshot` — **parcialmente resuelto:** el índice de combinaciones parciales (§10.3, pares/tríos de especies) no depende del spread, así que no está bloqueado. Pero "Repartos habituales" (`vFoe()`, hoy alimentado por `meta.json` estimado a mano) **no puede volverse 100% real solo con Limitless** — necesita Pikalytics (fuente secundaria, si cubre ese agregado) o seguir siendo una estimación marcada como tal. Esto hay que decidirlo explícitamente con Angel antes de tocar el modelo de datos consumido por el Motor — no es una ambigüedad que convenga resolver por default técnico.
