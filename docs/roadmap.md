...
- Captura OCR del equipo propio: **no funcionó** — sin logs del dispositivo no se pudo encontrar la causa exacta. Se endureció en dos frentes: match difuso (`closestMatch()`/`editDistance()`) para que un carácter mal leído no tire la lectura entera a la basura, y diagnóstico visible (cuántas líneas leyó ML Kit, sample crudo por tarjeta) para que el próximo intento deje información real en vez de fallar en silencio. Sigue sin verificarse contra el juego.
