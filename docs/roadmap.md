...
1. **Unificar a slugs canónicos en inglés + construir la capa i18n con selector de idioma** (`audit.md` §5.2 y §5.5, decisión #7 y #17). Hoy `calc()` compara contra 15+ literales en español; `build_dex.py` entrega inglés; no existe ninguna tabla `STRINGS` ni función `t()`. El trabajo es uno solo: introducir slugs canónicos (`intimidate`, no `"Intimidación"`) en el motor de daño y en las tablas `IT`/`ABIL`/`MV`, con una tabla de traducción `{slug: {en, es}}` resuelta por `t(clave, idioma)`, y un selector de idioma persistido (sugerido por el idioma del sistema al primer uso, cambiable después). *Verificación:* generar `dex.json` con `build_dex.py` y correr un combate con una habilidad clave (p. ej. Intimidate/Intimidación) — el modificador se aplica igual en ambos idiomas, y todo el texto de la interfaz cambia al conmutar el selector, sin ningún literal hardcodeado sobreviviente.
2. **Regenerar `meta.json`** (`audit.md` §5.3) para que coincida con las tablas canónicas `IT`/`ABIL` ya corregidas (sin ítems ni habilidades inexistentes en Champions), ahora usando slugs en vez de nombres en español. La re-auditoría dejó la lista exacta de entradas huérfanas (15 ítems, 5 habilidades) como punto de partida. *Verificación:* cada entrada de `meta.json` resuelve contra un ítem/habilidad real de las tablas canónicas — este chequeo puede ser el primer caso de uso del validador del punto 5.
...
5. **Escribir el script de validación de datos en build** (`architecture.md` §6): verifica que `meta.json`, `dex.json`, `sprite_index.json` y las tablas embebidas son mutuamente consistentes. Se apoya en el trabajo del punto 2 y sirve para detectar automáticamente una futura desincronización.
6. **Dejar una suite de pruebas mínima en el repo** (no scripts ad-hoc descartables, `audit.md` §5.7): como mínimo, un caso para el motor de daño con una habilidad clave (cubriría 5.2 si se reintrodujera) en los dos idiomas, y el validador del punto 5 corriendo como test.
